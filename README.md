...
本项目是基于 Qt 5.12 的串口配置工具，用于搜索、连接、管理和配置 WQ/NPort 串口设备。主要功能包括设备搜索、网络配置、串口参数配置、端口工作模式配置、端口管理、设备管理和 Linux Real COM 串口映射管理。
...
不要使用 `cmdPort = dataPort + 1`。这种旧规则会生成错误的映射，例如 `950/951`，设备节点可能存在但串口通信不可用。
...
1. 串口映射操作涉及系统目录和设备节点，需要 root 权限。
